...
<Form.Item name="tipo" label="Tipo de Cliente" rules={[{ required: true }]}>
                  <Select disabled={formBloqueado} onChange={(value) => {
    setTipoCliente(value);
    const cnpjValue = form.getFieldValue('cnpj');
    if (cnpjValue) {
...
<Form.Item name="cnpj" label="CPF/CNPJ" rules={[
        { required: true },
        { validator: (_, value) => {
            const tipo = form.getFieldValue('tipo');
            const valorLimpo = (value || '').replace(/\D/g, '');

            if (tipo === 'F' && valorLimpo.length !== 11) {
              return Promise.reject("CPF inválido para Pessoa Física");
            }

            if (tipo === 'J' && valorLimpo.length !== 14) {
              return Promise.reject("CNPJ inválido para Pessoa Jurídica");
            }

            if (!validarCNPJouCPF(valorLimpo)) {
